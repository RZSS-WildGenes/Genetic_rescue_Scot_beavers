...
# Test out effect of MAF filtering on KING results

```{bash}
# note have to include each individual only once, so using the structure file as input. 

plink --file input_data/BEV_V6_10_22_STRUCTURE --allow-extra-chr --recode vcf-iid --out input_data/BEV_V6_10_22_STRUCTURE # create standard vcf - 1451 SNPs


# Now filter for MAF
plink --file input_data/BEV_V6_10_22_STRUCTURE --allow-extra-chr --maf 0.1 --recode vcf-iid --out input_data/BEV_V6_10_22_STRUCTURE_maf0.1 # 1391 SNPs


plink --file input_data/BEV_V6_10_22_STRUCTURE --allow-extra-chr --maf 0.2 --recode vcf-iid --out input_data/BEV_V6_10_22_STRUCTURE_maf0.2 #1078 SNPs


plink --file input_data/BEV_V6_10_22_STRUCTURE --allow-extra-chr --maf 0.3 --recode vcf-iid --out input_data/BEV_V6_10_22_STRUCTURE_maf0.3 # 580 SNPs
...
~/tools/ngsRelate/ngsRelate -h input_data/BEV_V6_10_22_STRUCTURE_maf0.1.vcf -z Kinship/popmap_ngsRelate -T GT -c 1 -O Kinship/BEV_V6_10_22_STRUCTURE_maf0.1.ngsRelate

~/tools/ngsRelate/ngsRelate -h input_data/BEV_V6_10_22_STRUCTURE_maf0.2.vcf -z Kinship/popmap_ngsRelate -T GT -c 1 -O Kinship/BEV_V6_10_22_STRUCTURE_maf0.2.ngsRelate

~/tools/ngsRelate/ngsRelate -h input_data/BEV_V6_10_22_STRUCTURE_maf0.3.vcf -z Kinship/popmap_ngsRelate -T GT -c 1 -O Kinship/BEV_V6_10_22_STRUCTURE_maf0.3.ngsRelate
```
...
king_maf1 <- fread("Kinship/BEV_V6_10_22_STRUCTURE_maf0.1.ngsRelate", header=T) %>% 
  select(ida, idb, R0, R1, nSites, KING) %>% 
  left_join(select(pops, ida, PopA, Pop2A)) %>% 
  left_join(select(pops, idb, PopB, Pop2B)) %>%
  mutate(maf = 0.1)

king_maf2 <- fread("Kinship/BEV_V6_10_22_STRUCTURE_maf0.2.ngsRelate", header=T) %>% 
  select(ida, idb, R0, R1, nSites, KING) %>% 
  left_join(select(pops, ida, PopA, Pop2A)) %>% 
  left_join(select(pops, idb, PopB, Pop2B)) %>%
  mutate(maf = 0.2)
  
king_maf3 <- fread("Kinship/BEV_V6_10_22_STRUCTURE_maf0.3.ngsRelate", header=T) %>% 
  select(ida, idb, R0, R1, nSites, KING) %>% 
  left_join(select(pops, ida, PopA, Pop2A)) %>% 
  left_join(select(pops, idb, PopB, Pop2B)) %>%
  mutate(maf = 0.3)
...
king <- 
  rbind(king_maf0, king_maf1, king_maf2, king_maf3) %>% 
...
king_summ <- 
  rbind(king_summ0, king_summ1, king_summ2, king_summ3) %>% 
  left_join(cols_all) %>% 
...
plot_king_maf <-
king_summ %>% 
  ggplot(aes(maf, mean, colour=Pop_same2))+
  geom_errorbar(aes(ymin=CI.ll, 
                    ymax=CI.hl), 
                colour="black", width=.2)+
  geom_point(size=3)+
  facet_wrap(~(reorder(Pop_same2, Pop_order)),scales="free_y") +
  ylab(expression("Mean kinship (KING)"))+
...
```{r, eval = F}
ggsave2("Kinship/plot_King_varying_maf_meanCI.jpg",
        plot_king_maf,
